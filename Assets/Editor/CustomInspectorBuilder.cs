...
public class CustomInspectorBuilder : EditorWindow {
	...
	public void ShowMenu(int ID) {
		switch (ID) {
			case 0: {
					EditorGUILayout.LabelField("<color=#FFFFFF>Create or edit custom inspectors</color>", new GUIStyle { alignment = TextAnchor.MiddleCenter, richText = true });
			...
			case 3: {
					...
					if (type.BaseType == typeof(MonoBehaviour)) {
						...
						string path = Application.dataPath + "/Editor/" + EdittedFile + "PropertyDrawer.cs";
						...
						if (CreateMode) {
							string file =
								...
								"public class " + EdittedFile + "PropertyDrawer" + " : PropertyDrawer {\n" +
								...
								"		return 18 + CustomInspectorUtility.GetPropertyHeight(property, label, InspectorVariables);\n" +
								"	}\n" +
								"}\n" +
								"#endif";
...
public class RenameEditor : PropertyDrawer {
	...
	private bool ShowMe(SerializedProperty property) {
		...
		if (verificationResults[verificationResults.Count - 1] && checkType == 0) return true;
		else return false;
	}
	...
	public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
		...
		if (!ShowMe(property) && drawIf.hideVariable) return 0f;
